# 图 02 · 副本 · 掉落 · 任务闭环

> 用途：讲清"进副本 → 打死怪 → 掉东西 → **两端看到同一份**"这条闭环，
> 以及**任务系统现在接在哪、还没接在哪**（不粉饰）。
> 渲染好的图：`out\02-副本·掉落·任务闭环-1.svg`、`-2.svg`。

---

## 一、闭环主链路（进房 → 打死 → 掉落 → 两端一致）

```mermaid
sequenceDiagram
    autonumber
    participant A as 客户端 A
    participant B as 客户端 B
    participant S as 服务端权威世界
    participant D as DropTable.csv（真源）

    A->>S: JoinRoomRequest{room_id=1001}
    S->>S: FromDungeon(1001)：狼巢 / 半径 3000mm / 野狼 6001 x2（各 300 血）+ 狼王 6003（2000 血）
    S-->>A: RoomState（席位表）
    S-->>B: RoomState（席位表）

    A->>S: PlayerInput 且 action_bits 第 0 位 = 普攻
    S->>S: 射程够吗（2000mm）？冷却好了吗（15 帧 = 0.5 秒）？
    S->>S: DamageMath.Resolve：剑士普攻 100 点（Hero.skillIds 第一项 = 2001 火球，伤害 100）
    S-->>A: ServerEvent{Damage}
    S-->>B: ServerEvent{Damage}
    Note over S: 第 3 发把野狼从 300 打到 0<br/>只有从活着打到死那一下才掷掉落，打尸体不重复掉
    S->>D: 查 6001 的掉落行
    D-->>S: 9001 x1~2 @10000、9002 x1~1 @10000
    S->>S: BattleRandom（xorshift32）按万分比掷骰，数量在 countMin..countMax 之间
    Note over S: 每条掉落事件只序列化一次<br/>把同一份字节发给房里每个人
    S-->>A: ServerEvent{Drop}
    S-->>B: ServerEvent{Drop}
    A->>A: NetSession.DropReceived / RecentDrops（最多 20 条）
    B->>B: NetSession.DropReceived / RecentDrops（最多 20 条）
    Note over A,B: 两端看到的是同一份字节<br/>而不是两边各掷一次、结果正好一样
```

**三个值得讲的取舍**：

| 取舍 | 怎么做 | 为什么 |
| --- | --- | --- |
| 掷骰用什么随机数 | 自己写 `BattleRandom`（xorshift32），不用 `System.Random` | `System.Random` 的算法**跨 .NET 版本变过**；掉落是"两端必须一致"的东西，随机数一旦不可复现，单端测试全绿、联机时才表现为"两边掉的不是同一个" |
| 谁掷骰 | **只有服务端**掷，然后把结果发下去 | 客户端不掷 = 不给作弊留口子；也不需要"同步随机种子"那套 |
| 掉落归谁 | `DropEvent.WinnerPlayerId`（M3 = 击杀者） | 把"归属"留在协议里，M4 做组队分配时不用改协议结构 |

---

## 二、任务闭环：现在通到哪、哪里是断的（**这张图很重要**）

```mermaid
flowchart LR
    BW["M2 本地战斗<br/>BattleWorld：扣血 · 致死 · 技能命中"]
    EC["EventCenter<br/>BattleEvents.DamageDealt / MonsterDied / SkillHit<br/>WorldEvents.AreaEntered"]
    BR["ConditionEventBridge（那座桥）<br/>Bind：事件 到 条件事件<br/>KillMonster / UseSkill / ReachArea"]
    CT["ConditionTracker（共享层 · 无引擎依赖）<br/>Notify(事件类型, 目标编号, 次数)"]
    QR["QuestRuntime<br/>接取 到 进度 到 完成 到 交付 到 发奖"]
    NS["M3 网络路径（客户端）<br/>NetSession：收到 Damage / Death / Drop 事件"]

    BW --> EC
    EC --> BR
    BR --> CT
    CT --> QR
    NS -.->|"M4 待接：把服务端事件翻译成 EventCenter 事件（现在没接）"| EC
```

**这张图的诚实结论（面试时主动说出来，比被问到强）**：

- **已经通的**：M2 的本地战斗 → `EventCenter` → `ConditionEventBridge` → `ConditionTracker` → `QuestRuntime`，
  这条链路是完整闭环，EditMode 里有用例钉着（`ConditionTrackerTests` 27 条、`QuestRuntimeTests` 24 条）。
  **任务与成就共用同一个 `ConditionTracker`** —— 这是 M4 做成就时的直接复用点。
- **还没接的**：M3 的**联机**战斗结果目前只到 `NetSession`（`DropReceived` → 调试窗口显示），
  **没有翻译成 `EventCenter` 事件**。也就是说：现在联机打死怪，任务进度**不会**涨。
  这不是 bug 没修，而是**下一片要干的活**（M4 的第一件事）：
  在客户端加一个"服务端事件 → EventCenter"的适配，`ConditionEventBridge` 与任务模块**一行都不用改**。
- 为什么要**现在**把这条断口画出来：它决定了 M4 的接线方式 —— 适配层放在 `NetSession` 与 `EventCenter` 之间，
  **任务系统继续不认识网络**（和它不认识 `EventCenter` 是同一个设计：每层只认识相邻那层）。

---

## 三、这一段踩过的两个坑（都是"改了却没生效"）

| 现象 | 根因 | 防复发（已落成代码/流程） |
| --- | --- | --- |
| **打死了什么都不掉**（且不报错） | 服务端读的是 `Configs\Design\DropTable.csv`（**真源**），而当时只改了 Unity 侧的 SO `DropTableConfig.asset` → 服务端那 5000/3000（35% 什么都不掉） | 服务端**启动时逐条念出它真正读到的掉落**：`掉落：怪 6001 → 物品 9001 x1~2@10000、…` |
| **重启服务端后新日志没出现** | 跑的是**旧的 exe**（08:56 编的），而配置表接入是 09:34、掉落是 09:58 —— 那个 exe 里**既没有配置表也没有掉落**，所以"看不到狼"和"不掉东西"**是同一个根因** | 启动横幅打印 `Built :`（上一次编译时间）+ **源码比产物新就大声报警** |

> 📌 这两条现在都在 `Docs\排查手册\06-改了却没生效（旧产物·生成物·真源）.md` 里，含四个实测对照。

---

## 四、图里每个数字的出处

| 事实 | 值 | 出处 |
| --- | --- | --- |
| 副本 1001 | 狼巢 / 4 人 / 半径 3000mm / 怪 `6001,6001` / BOSS 6003 | `Configs\Design\Dungeon.csv` |
| 野狼 6001 | HP 300 / 攻击 20 / 技能 `2003` | `Configs\Design\Monster.csv` |
| 狼王 6003 | HP 2000 / 攻击 60 / 技能 `2001,2002` | 同上 |
| 剑士 1001 | HP 1200 / 移速 5000mm 每秒 / 技能 `2001,2002` | `Configs\Design\Hero.csv` |
| 普攻伤害 | 100（= `Hero.skillIds` 第一项 2001 火球） | `Skill.csv` + `DungeonBattle.FromDungeon` |
| 狼 6001 掉落 | 9001 x1~2 @10000、9002 x1~1 @10000（万分比，10000 = 必掉） | `Configs\Design\DropTable.csv` |
| **只有从活着打到死才掷骰** | `wasAlive && !target.Alive` | `Server\NBC.Server.Game\DungeonBattle.cs` |
| 掉落事件序列化次数 | 每条事件 1 次，然后发给全房 | `Server\NBC.Server.Game\RoomBattleService.cs` `BroadcastDrops` |
| 任务/成就共用条件系统 | 同一个 `ConditionTracker` | `Client\Assets\_Project\Game\Quest\QuestRuntime.cs` 文件头 |

---

## 五、面试版（30 秒）

> 副本这一段的核心是"**同一份字节**"：怪一死，服务端用自己那份 `BattleRandom`（xorshift32，
> 不用 `System.Random`，因为它跨 .NET 版本换过算法）按万分比掷骰，然后把掉落**序列化一次**、
> 同一份字节发给房里所有人 —— 两端一致是**保证**出来的，不是"各算一遍碰巧一样"。
> 掷骰只在"从活着打到死"那一下发生，打尸体不重复掉。
> 任务那边我把边界画清楚了：M2 的本地战斗已经通过 `ConditionEventBridge` 接进 `ConditionTracker`，
> 但 M3 联机战斗的结果目前还停在 `NetSession`，**没有进事件中心**，所以联机打死怪任务不涨进度；
> 这是 M4 第一件要接的线，接法是在 `NetSession` 与 `EventCenter` 之间加一层适配 ——
> 任务模块一行都不用改，因为它至今不认识网络。
