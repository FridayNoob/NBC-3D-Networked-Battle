...
`ExportPipeline` 是**唯一把这些接缝串起来**的地方，它自己**不碰文件系统**
（产物是 `EmittedFile`，落盘由宿主决定），所以整条流水线能在内存里跑、能被自测逐字断言。
数据流图强调的是那条硬语义：校验有错就 `return`，**一个文件都不产出**。
...
| 容易画错 | 事实 | 证据 |
| --- | --- | --- |
| 以为 `ExportPipeline` 自己写文件 | 它**不碰文件系统**，只产出 `EmittedFile`（相对路径 + 内容） | `Pipeline.cs:6-12` |
| 以为 `ExportPipeline` 是 `static` 工具类 | 它是**实例类**，构造时注入 `ConfigPolicy` + `ITableEmitter` + `EmitOptions` | `Pipeline.cs:151`、`:161-166` |
| 以为 `ConfigPolicy` 是个 `enum` | 它是**类**（策略是**数据**不是代码） | `Policy.cs:41`、`:54`、`:57`、`:102` |
| 以为诊断是「一句拼好的字符串」 | `DiagnosticBag` 收的是**结构化**诊断；排版是可换的 | `Diagnostics.cs:254`、`Pipeline.cs:96-98` |
