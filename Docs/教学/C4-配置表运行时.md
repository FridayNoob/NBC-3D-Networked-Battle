...
`ConfigMgr` **只持有一个 `IConfigSource` 字段**，所以它从头到尾不认识资源系统；
真实现 `AssetConfigSource` 才去依赖 `AssetManager`。数据流图把**两条消费路径**画在一起：
服务端直接读 CSV 真源，Unity 读的是生成物 SO —— 这两条分叉是这一章最值钱的一张图。
...
| 容易画错 | 事实 | 证据 |
| --- | --- | --- |
| 以为 `ConfigMgr` 与 `<表>Config` 之间有接口或抽象基类 | **没有编译期契约**：只有命名约定（`HeroConfig ↔ Hero`）+ `where T : ScriptableObject` | `ConfigMgr.cs:102`、`:259`、`:46` |
| 以为 `Config_<表>` 是**泛型基类** | 它是**每张表一个具名类**（`Config_Hero`），不是泛型 | `Config_Hero.cs:11`、`CodeGen.cs:126` |
| 以为 `Config_<表>` 是手写的 | 是**生成物**，头三行就是 `auto-generated` 警告 | `Config_Hero.cs:1-3`、`HeroConfig.cs:1-3` |
| 以为 `ConfigMgr` 认识资源系统 | 它的 `using` 里**没有** `NBC.Framework.Asset`；`AssetManager` 只出现在 `AssetConfigSource` | `ConfigMgr.cs:34-38`、`AssetConfigSource.cs:46` |
| 以为用完会释放配置句柄 | 句柄**常驻不 `Dispose`**（配置表进程内常驻，有意为之） | `AssetConfigSource.cs:34-36`、`:136` |
| 以为判成功看「句柄非 null」 | 用 `IsValid`（= 成功 **且** 资产非 null） | `AssetConfigSource.cs:38-41`、`:129` |
| ⚠️ **以为「有 9 张表」就等于「启动预加载 9 张」** | **两个不同的集合**：`GameTables.All` 只有 **7** 张（Hero / Skill / Level / Monster / Quest / QuestCondition / Reward），`Generated\` 与 `Configs\` 里却有 **9** 张表的产物（多出 `Dungeon` / `DropTable`） | `GameTables.cs:33-45`；`dir` 实测 9 个 `*Config.cs`、9 个 `.tsv`、9 个 `.asset` |

> 📌 最后一条值得单独记住：**「有哪些表」是生成器的产物清单，「启动预加载哪些表」是游戏自己的需求清单**
> —— 后者就是 `GameTables.All` 这一个数组，加表时只改它。`Dungeon` / `DropTable` 目前只有服务端在用。
