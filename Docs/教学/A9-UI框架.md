...
`UIManager` 与 `BasePanel` 是**组合**（管理器持有面板字典），不是继承；层配置搬到了
`UILayers` 组件上，所以管理器只**依赖**它。后面两张图说明一件事：
**`HidePanel`（回池）与 `ClosePanel`（销毁）是两条不同的生命周期。**
...
classDiagram
    ...
    QuestPanel ..> QuestPanelModel : 依赖 Bind 的入参
```
...
| 以为 `BasePanel` 靠 `Awake` 初始化 | 它**没有** `Awake`；`Initialize()` 是 **public**（EditMode 要直接调） | `BasePanel.cs:34-45`、`:107` |
| 以为控件按 `transform.Find` 找 | 按 **GameObject 名字**索引，且一次全树扫 `UIBehaviour` | `BasePanel.cs:140`、`:129` |
| 以为 `HidePanel`／`ClosePanel` 只有强弱之分 | 是**两条生命周期**：回池（不销毁）vs 销毁 + 释放句柄 | `UIManager.cs:395-405`、`:431` |
| 以为 `LoadingMaskController` 是单例 | 它是**普通类**，由游戏层决定何时 `Attach()` | `LoadingMaskController.cs:43` |
...
flowchart TD
    ...
    C -- 否 --> E["EnsureCanvas 引导 Canvas"]
    E --> F["AssetManager.LoadAssetAsync 取 Canvas"]
    F --> G["OnCanvasLoaded 实例化并校验四层"]
    G --> H["StartPanelLoad 用前缀拼面板地址"]
    H --> I["OnPanelLoaded"]
    I --> J["Instantiate 原始 prefab 后取 BasePanel"]
    J --> K["AttachToLayer 挂到目标层"]
    K --> L["panel.Initialize()"]
    L --> M["CollectControls 一次扫全部 UIBehaviour"]
    M --> N["OnInit 子类钩子 然后 panel.ShowMe()"]
    N --> P["SucceedPending 逐个回调"]
```
...
它持有面板与素材句柄，但**不认识**任何具体面板 —— 具体面板只通过 `BasePanel` 的
`Initialize()` / `ShowMe()` / `HideMe()` 被驱动。关键一句：**关面板是收起来回池、不是销毁**，
