...
## 七、中间产物为什么是 **TSV + JSON 两份**（⚙️ 面试可以直接讲的一段）

> 这一节回答一个必然会被问的问题：**"为什么不用 JSON？别的项目都用 JSON 或二进制。"**

答案是**先分清边界** —— 同一个项目里，不同的边界该用不同的格式：

| 边界 | 格式 | 为什么 |
| --- | --- | --- |
| **网络协议** | **二进制**（`Google.Protobuf`） | 带宽/解析开销敏感、要跨语言、要 schema 演进。服务端已在用 |
| **运行时存档 / 设置 / 跨端** | **JSON** | 要人能读、要能容忍字段增减 |
| **构建期「表 → 资产」交接** | **TSV（给导入器）＋ JSON（对外）** ← 本节 | 见下 |

### 7.1 构建期这一格为什么由 TSV 打头

不是偏好，是**边界性质**决定的：

| 这个边界的特点 | JSON 的优势 | TSV 的优势 |
| --- | --- | --- |
| 消费者是**我们自己生成的加载器**，**它已经知道每个字段的类型** | ❌ "自描述结构"用不上 | ✅ 按行/按名取字段就够 |
| **同一台机器、构建期**只跑一次 | ❌ 解析性能无所谓 | ✅ `string.Split` 即可，**零解析库** |
| 生成物**要入库**、要看 diff | ➖ 也不差 | ✅ **按行 diff** 干净 |
| 数据**严格是表格**（行 × 列） | ➖ | ✅ 天然形状 |

### 7.2 那为什么**还要**出 JSON（2026-09-22 决定，`--emitter all` 默认出三种）

原计划只做 JSON 让 Unity 用 `JsonUtility` 读。动手前量清两个坑：

| 坑 | 说明 |
| --- | --- |
| **Unity 序列化不支持可空值类型** | `int?` 会**静默不序列化**（丢数据不报错），而规范允许 `ref:Hero?` |
| **`JsonUtility` 里 enum 按整数走** | 枚举定义在 C# 里，**工具不知道成员数值**，只能写成员名 → 装不进去 |

⚠️ 而且这两条**我在本机无法实测**（没有 Unity），工程里也**没有 Newtonsoft**（`manifest.json` 实测只有 `com.unity.modules.jsonserialize`）。
**赌一个自己验不了的行为，正是本项目最反对的事。**

于是改成：**导入器吃 TSV**（Unity 侧只要 `string.Split`，零解析库，且 TSV 文本**我能在这里逐字断言**），
**JSON 作为对外产物**补上它独有的价值：

| 诉求 | TSV 行不行 |
| --- | --- |
| 外部工具 / 别的语言 / 别的引擎要读 | ⚠️ 得先教它 TSV 是什么 |
| 评审时人直接看得懂 | ⚠️ 要解释 |
| 以后配置出现**嵌套结构**（`Vector3`、子表、参数字典） | ❌ **扁平格式装不下** ← TSV 的**真限制** |

📌 **两份都要，不是二选一** —— 这正是 `ITableEmitter` 这个接缝的用处：**加一种产物 = 加一个文件**。

### 7.3 三份产物必须**给出同一个答案**

同一张表，C# 字段 / TSV / JSON 对"没填的可空 ref"必须说同一件事，否则换一份产物读就会得到不同的值 —— **静默不一致**。约定如下（**有自测逐条钉住**）：

| 类型 | C# 字段 | TSV | JSON |
| --- | --- | --- | --- |
| `ref:Buff?` 留空 | `int`（**不是 `int?`**） | 空串 | **`0`**（0 = 无引用，主键永远 ≥ 1） |
| `string?` 留空 | `string` | 空串 | **`null`** |
| `ref:Skill[]?` 留空 | `int[]` | 空串 | **`[]`** |

> ⚠️ 枚举在 JSON 里写**成员名**（`"Fire"`）：工具不知道成员的数值，消费者要做一次名字→值转换（生成的加载器就是这么做的）。

**顺带逼出一条新的校验规则（`CFG0019`）**：可空只允许 `ref:` / `ref:[]` / `string` ——
可空 `int?` / `float?` / `bool?` / `long?` **一律报错**（生成 `int?` 会静默丢数据，退化成 `int` 又让"没填"和"填了 0"分不开）。
> 这是对 `Docs\17` §六 的**收窄**（原写"任意类型加 `?`"），按 §十二 约定的做法记一笔。

### 7.4 ⚠️ 手写 JSON 的可信度（以及怎么解决）

JSON 产物是**手写的**（Core 零第三方依赖，不引 Newtonsoft）—— 那"它合法"凭什么算数？
**不算数，除非用真解析器验一遍。** 所以自测里直接拿 .NET 8 自带的 `System.Text.Json`
**反过来解析**我生成的 JSON：转义漏了、逗号多写了、括号不配平**都会当场抛**；
再加一条"转义后还原出来的字符串**一个字符不差**"。
...
| `ConfigKit.Sources.Delimited`（CSV / TSV 读入） | ✅ |
| `ConfigKit.SelfTest`（自测运行器 + 架构守卫） | ✅ **48 条全绿**（零依赖） |
| `ConfigKit.Core` 代码生成（`Config_*.cs` + `<表>Config.cs`） | ✅ |
| `ConfigKit.Cli`（命令行 + 退出码 0/1/2 + `--format` + `--emitter`） | ✅ |
| `ConfigKit.Sources.Xlsx`（NPOI） | ✅ **9 条探针全绿**（真 xlsx 读写） |
| `ConfigKit.XlsxProbe`（可复现验证探针 + 示例表生成） | ✅ |
| TSV 发射器 + 生成的 `LoadFromTsv` 加载器 | ✅ |
| JSON 发射器（对外产物，含真解析器反向校验） | ✅ |
...
| # | 局限 | 说明 / 什么时候补 |
| --- | --- | --- |
| 1 | **TSV 是扁平格式，装不下嵌套结构** | 这是选它当导入器输入的**真代价**。以后配置里出现 `Vector3`、子表、参数字典时，**TSV 表达不了** —— 那时要么换 JSON 当导入器输入（JSON 产物已经在了），要么给嵌套部分单独设计。**现在 3 张表全是扁平的，所以没撞上** |
| 2 | **JSON 是手写的**（Core 零依赖，不引 Newtonsoft） | 合法性由 `System.Text.Json` 反向解析保证；但**没有**做 schema 校验（那是消费者的事）。若将来需要严格 schema，加 Newtonsoft.Json.Schema 之类 |
| 3 | **可空只支持 `ref` / `ref[]` / `string`**（`CFG0019`） | 被 Unity 序列化限制逼出来的收窄，不是设计偏好。若换掉 Unity 侧消费者（比如服务端共用），可以放开 |
...
| 3 | **不做外键环检测** | M1 只做存在性（`Docs\17` §十一 已如实记录）。⚠️ 2026-09-26 补：**跨表检查**（§十一）已经落地三条，但**环检测仍然不做** |
| 7 | **跨表检查只覆盖"任务 ↔ 副本 ↔ 掉落 ↔ 事件源"三类** | 见 §十一。它们解决的是"单表全绿、合起来玩不成"，但**不是通用的跨表约束系统** —— 加新规则要写代码（不是配数据），目前只有这三条 |
...
## 十一、跨表检查（2026-09-26 新增：**单表全绿 ≠ 这张表能被玩通**）

### 11.1 缘起（真事，不是假想）

M3 收关、M4-S1 实跑验收通过之后，我读**真任务表**想接真任务 —— 结果发现
**联机路径下 4 个任务一个都完不成**，而 `ConfigKit` 的报告是 **0 错误 0 警告**：

| # | 表 A 要什么 | 表 B 给什么 | 后果 |
| --- | --- | --- | --- |
| 1 | `QuestCondition` 要 `KillMonster 6001 × 3` | `Dungeon` 1001 只刷 **2** 只（`"6001,6001"`） | 条件**永远达不成** |
| 2 | 任务要 `CollectItem 7001`（狼皮） | `DropTable` **从不掉 7001**；7001 只在**它自己那份奖励**里 | 循环依赖：要 2 张狼皮才给 2 张狼皮 |
| 3 | 任务的 `ReachArea` 条件 | 联机路径**没有区域事件源** | 条件**永远不涨** |

原因不是"校验器有 bug"，而是**判据只覆盖了一半**：
`range` / `len` / `unique` / `ref:存在性` 全是**单表**判据 —— 每一张表单看都自洽，
**合起来却什么都玩不成**。这与 `Docs\00` 那条"**闸门绿 ≠ Unity 绿**"是同一族。

### 11.2 四条规则（`CrossTableChecks`，跑在单表校验之后）

| 编号 | 规则 | 级别 |
| --- | --- | --- |
| `CFG0020` | `KillMonster X × N`：必须有**某个副本**能提供 **≥ N 只** X（`Dungeon.monsters` 的重复次数 + `bossId`）<br>⚠️ 只对**任务**引用、且 `targetId > 0` 的条件成立（见下） | **错误** |
| `CFG0021` | `CollectItem I`：I 必须出现在 `DropTable.itemId`，**或**出现在**别的**持有者（任务/成就）的奖励里。<br>⚠️ 只在"**它自己那个任务**的奖励"里 = **循环依赖** ⇒ 判为拿不到 | **错误** |
| `CFG0022` | 用到了"当前**没有事件源**"的事件类型（列表在 `ConfigPolicy.EventTypesWithoutSource`） | **警告**（这是实现缺口，不是数据写错） |
| `CFG0023` | 一条 `QuestCondition` 被**多个持有者**引用（两个任务 / 任务 + 成就）<br>⇒ 运行期 `ConditionTracker.Register` **直接抛 `InvalidOperationException`** | **错误** |

**为什么 `CFG0021` 的判据要绕这一下**：第一版只写"出现在掉落或奖励里"，
结果被 `Reward` 里那两条"自己给自己"的奖励**骗过去了**（`7001`/`7002` 都在奖励里）
—— 是**自测的阳性对照当场抓出来的**。判据必须与要抓的现象匹配：
"能拿到"不等于"某张表里提到过它"。

**为什么 `CFG0020` 要排除两种情况**（2026-09-26 加 `Achievement` 表时被**误报逼出来的**）：
...
> 真表里「初出茅庐 · 累计击杀 10 只野狼」和「猎手的直觉 · 累计击杀 5 只任意怪」
> 这两条就是这么被误报成错误的。
> 两条豁免都有**能证伪它的阴性对照**（见 `Program.cs` 里的变异测试结论：去掉任一条，对应用例当场变红）。

### 11.3 调用点只有一处（`ConfigChecks.RunAll`）

导出流水线（`ExportPipeline.Run`）与**自测**（`ConfigKit.SelfTest` 的 `Validate`）
都调 `ConfigChecks.RunAll(set, policy, diagnostics)` ——

> 📌 原来自测是**直接调 `ValidationEngine`** 的，于是"流水线查了、自测没查"：
> 新加的跨表检查**在自测里根本不跑**（自测就成了假绿）。这与
> `Docs\00` 那条"**真实检查与工具的真实判定必须走同一个来源**"是同一个道理。

### 11.4 实测

| 项 | 结果 |
| --- | --- |
| 自测 | **58 条全绿**（跨表新增 9 条：CFG0020 阳/阴×3、CFG0021 阳/阴、CFG0022、CFG0023×2） |
| 阳性对照（未对齐的真表） | 报 **3 错 1 警**（2×CFG0020 + 1×CFG0021 + 1×CFG0022），**未产出任何文件** |
| 对齐后（真表，含 `Achievement`） | **0 错 0 警告**，写出 **40 个文件** |
