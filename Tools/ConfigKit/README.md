...
### 11.2 三条规则（`CrossTableChecks`，跑在单表校验之后）

| 编号 | 规则 | 级别 |
| --- | --- | --- |
| `CFG0020` | `KillMonster X × N`：必须有**某个副本**能提供 **≥ N 只** X（`Dungeon.monsters` 的重复次数 + `bossId`） | **错误** |
| `CFG0021` | `CollectItem I`：I 必须出现在 `DropTable.itemId`，**或**出现在**别的**任务的奖励里。<br>⚠️ 只在"**它自己那个任务**的奖励"里 = **循环依赖** ⇒ 判为拿不到 | **错误** |
...
| 项 | 结果 |
| --- | --- |
| 自测 | **54 条全绿**（新增 5 条：CFG0020 阳/阴、CFG0021 阳/阴、CFG0022） |
| 阳性对照（未对齐的真表） | 报 **3 错 1 警**（2×CFG0020 + 1×CFG0021 + 1×CFG0022），**未产出任何文件** |
| 对齐后（真表） | **0 错 0 警告**，写出 **36 个文件** |
