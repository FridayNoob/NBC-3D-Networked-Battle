// <auto-generated />
// 由 ConfigKit 生成：Achievement 表的配置资产。
...
{
    /// <summary>Achievement 表的配置资产。</summary>
    [CreateAssetMenu(fileName = "AchievementConfig", menuName = "配置/Achievement")]
    public sealed class AchievementConfig : ScriptableObject
    {
        public List<Config_Achievement> rows = new List<Config_Achievement>();  // 全部数据行
        private Dictionary<int, Config_Achievement> m_index;  // 主键索引（不序列化，加载时重建）
        ...
        {
            m_index = new Dictionary<int, Config_Achievement>(rows == null ? 0 : rows.Count);
        ...
        public Config_Achievement Get(int id)
        {
            ...
            {
                throw new KeyNotFoundException("[AchievementConfig] 没有主键 " + id);
            }
        ...
        public bool TryGet(int id, out Config_Achievement row)
        {
            if (m_index == null)
            {
                RebuildIndex();
        ...
        public void LoadFromTsv(string text)
        {
            ...
            for (int i = 1; i < lines.Length; i++)  // 从第 2 行开始（第 1 行是字段名）
            {
                ...
                row.desc = Unescape(Cell(cells, header, "desc"));
                row.conditionIds = ParseIntArray(Cell(cells, header, "conditionIds"));
                row.rewardId = ParseInt(Cell(cells, header, "rewardId"));
