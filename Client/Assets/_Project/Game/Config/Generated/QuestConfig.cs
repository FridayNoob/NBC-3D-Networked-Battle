...
{
    /// <summary>Quest 表的配置资产。</summary>
    [CreateAssetMenu(fileName = "QuestConfig", menuName = "配置/Quest")]
    public sealed class QuestConfig : ScriptableObject
    {
        public List<Config_Quest> rows = new List<Config_Quest>();  // 全部数据行
        private Dictionary<int, Config_Quest> m_index;  // 主键索引（不序列化，加载时重建）
        ...
        {
            m_index = new Dictionary<int, Config_Quest>(rows == null ? 0 : rows.Count);
        ...
        public Config_Quest Get(int id)
        {
            Config_Quest row;
            ...
            {
                throw new KeyNotFoundException("[QuestConfig] 没有主键 " + id);
            }
        ...
        public bool TryGet(int id, out Config_Quest row)
        {
            if (m_index == null)
            {
                RebuildIndex();
