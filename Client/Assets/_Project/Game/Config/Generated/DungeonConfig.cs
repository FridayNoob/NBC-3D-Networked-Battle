...
{
    /// <summary>Dungeon 表的配置资产。</summary>
    [CreateAssetMenu(fileName = "DungeonConfig", menuName = "配置/Dungeon")]
    public sealed class DungeonConfig : ScriptableObject
    {
        public List<Config_Dungeon> rows = new List<Config_Dungeon>();  // 全部数据行
        private Dictionary<int, Config_Dungeon> m_index;  // 主键索引（不序列化，加载时重建）
        ...
        {
            m_index = new Dictionary<int, Config_Dungeon>(rows == null ? 0 : rows.Count);
        ...
        public Config_Dungeon Get(int id)
        {
            Config_Dungeon row;
            ...
            {
                throw new KeyNotFoundException("[DungeonConfig] 没有主键 " + id);
            }
        ...
        public bool TryGet(int id, out Config_Dungeon row)
        {
            if (m_index == null)
            {
                RebuildIndex();
        ...
        public void LoadFromTsv(string text)
        {
            ...
            for (int i = 1; i < lines.Length; i++)  // 从第 2 行开始（第 1 行是字段名）
            {
                ...
                Config_Dungeon row = new Config_Dungeon();
                row.id = ParseInt(Cell(cells, header, "id"));
                row.name = Unescape(Cell(cells, header, "name"));
                row.maxPlayers = ParseInt(Cell(cells, header, "maxPlayers"));
                row.spawnRadiusMm = ParseInt(Cell(cells, header, "spawnRadiusMm"));
                row.monsters = ParseIntArray(Cell(cells, header, "monsters"));
                row.bossId = ParseInt(Cell(cells, header, "bossId"));
