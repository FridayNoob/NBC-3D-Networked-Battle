...
{
    /// <summary>Monster 表的配置资产。</summary>
    [CreateAssetMenu(fileName = "MonsterConfig", menuName = "配置/Monster")]
    public sealed class MonsterConfig : ScriptableObject
    {
        public List<Config_Monster> rows = new List<Config_Monster>();  // 全部数据行
        private Dictionary<int, Config_Monster> m_index;  // 主键索引（不序列化，加载时重建）
        ...
        {
            m_index = new Dictionary<int, Config_Monster>(rows == null ? 0 : rows.Count);
        ...
        public Config_Monster Get(int id)
        {
            Config_Monster row;
            ...
            {
                throw new KeyNotFoundException("[MonsterConfig] 没有主键 " + id);
            }
        ...
        public bool TryGet(int id, out Config_Monster row)
        {
            if (m_index == null)
            {
                RebuildIndex();
        ...
        public void LoadFromTsv(string text)
        {
            ...
            for (int i = 1; i < lines.Length; i++)  // 从第 2 行开始（第 1 行是字段名）
            {
                ...
                row.attack = ParseInt(Cell(cells, header, "attack"));
