...
{
    /// <summary>Reward 表的配置资产。</summary>
    [CreateAssetMenu(fileName = "RewardConfig", menuName = "配置/Reward")]
    public sealed class RewardConfig : ScriptableObject
    {
        public List<Config_Reward> rows = new List<Config_Reward>();  // 全部数据行
        private Dictionary<int, Config_Reward> m_index;  // 主键索引（不序列化，加载时重建）
        ...
        {
            m_index = new Dictionary<int, Config_Reward>(rows == null ? 0 : rows.Count);
        ...
        public Config_Reward Get(int id)
        {
            Config_Reward row;
            ...
            {
                throw new KeyNotFoundException("[RewardConfig] 没有主键 " + id);
            }
        ...
        public bool TryGet(int id, out Config_Reward row)
        {
            if (m_index == null)
            {
                RebuildIndex();
        ...
        public void LoadFromTsv(string text)
        {
            ...
            for (int i = 1; i < lines.Length; i++)  // 从第 2 行开始（第 1 行是字段名）
            {
                ...
                row.exp = ParseInt(Cell(cells, header, "exp"));
                row.gold = ParseInt(Cell(cells, header, "gold"));
                row.itemId = ParseInt(Cell(cells, header, "itemId"));
                row.itemCount = ParseInt(Cell(cells, header, "itemCount"));
